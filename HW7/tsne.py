...
def pca(x, no_dims=50):
    ...
    index = np.argsort(eigen_values)[::-1][:n] # Sort eigen value in descending order(largest to smallest)
...
if __name__ == "__main__":
    ...
    X = np.genfromtxt('mnist_X.csv', delimiter=',')
    labels = np.genfromtxt('mnist_label.csv', delimiter=',')
    Y = tsne(X, 2, 50, 20.0)
